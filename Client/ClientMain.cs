...
namespace Core.Client
{
    public class ClientMain : BaseScript
    {
        ...
        public ClientMain()
        {
            ...
            EventHandlers["core:openConcessNUI"] += new Action<string>(OnOpenConcessNUI);
            EventHandlers["core:closeNUI"] += new Action(OnCloseNUI);
            ...
            RegisterCommand("concess", new Action(() =>
            {
                Debug.WriteLine("[TEST] Commande testconcess exécutée");
                ...
                var testData = new
                {
                    categories = new[]
                    {
                        new
                        {
                            ...
                            vehicles = new[]
                            {
                                new { model = "ADDER", name = "Adder", price = 150000 }
                            }
                        }
                    },
                    colors = new[]
                    {
                        new { id = 0, name = "Noir" },
                        new { id = 27, name = "Rouge" }
                    }
                };

                string json = JsonConvert.SerializeObject(testData);
                Debug.WriteLine($"[TEST] JSON: {json}");

                NUIManager.Instance.OpenNUI("concessAuto", testData);
                Debug.WriteLine("[TEST] NUI ouvert");
            }), false);
            RegisterCommand("fix_cursor", new Action(() =>
            {
                API.SetNuiFocus(false, false);
                API.SetNuiFocusKeepInput(false);
                if (NUIManager.Instance.IsOpen) NUIManager.Instance.CloseNUI();
            }), false);

            RegisterCommand("debug_callbacks", new Action(() =>
            {
                Debug.WriteLine("\n╔════════════════════════════════════════════════════╗");
                Debug.WriteLine("║          DEBUG CALLBACKS NUI                       ║");
                Debug.WriteLine("╚════════════════════════════════════════════════════╝");

                // Vérifier l'état du NUIManager
                Debug.WriteLine($"NUIManager IsOpen : {NUIManager.Instance.IsOpen}");
                Debug.WriteLine($"NUIManager CurrentNUI : {NUIManager.Instance.CurrentNUI}");

                // Tester l'enregistrement d'un callback temporaire
                Debug.WriteLine("\n[TEST] Enregistrement d'un callback test...");
                NUIManager.Instance.RegisterCallback("test:callback", (data) =>
                {
                    Debug.WriteLine("[TEST] Callback test appelé !");
                });
                Debug.WriteLine("[TEST] Callback test enregistré");

                // Forcer React à envoyer le callback
                Debug.WriteLine("\n[TEST] Simulation d'un appel de callback...");
                Debug.WriteLine("Ouvrez la console navigateur (F12 dans le NUI) et tapez :");
                Debug.WriteLine("fetch('https://core/nuiCallback', {");
                Debug.WriteLine("  method: 'POST',");
                Debug.WriteLine("  headers: { 'Content-Type': 'application/json' },");
                Debug.WriteLine("  body: JSON.stringify({ callback: 'bank:onClose' })");
                Debug.WriteLine("})");
            ...
            var item1 = new LTDItems("Ordinateur", "Idéal pour faire du pentest...", 15000, "📱", "Électronique");
            NarcoItems.Add(item1);

            var item2 = new LTDItems("Perceuse", "Idéale pour percer une porte...", 10000, "📱", "Électronique");
            NarcoItems.Add(item2);

            var item3 = new LTDItems("Phone", "Excellent téléphone", 1500, "📱", "Électronique");
            NarcoItems.Add(item3);
            
            var item4 = new LTDItems("Outil de crochetage", "En cas de panne", 1000, "🔥", "Électronique");
        ...
        private void OnOpenConcessNUI(string jsonData)
        {
            try
            {
                var testDeserialize = Newtonsoft.Json.JsonConvert.DeserializeObject(jsonData);
                Debug.WriteLine($"[CORE] JSON valide");
                ...
                Debug.WriteLine("[CORE] NUI ConcessAuto ouvert avec succès");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[CORE] ERREUR: {ex.Message}");
            }
        }

        private void OnCloseNUI()
        {
            Debug.WriteLine("[CORE] OnCloseNUI");
            API.SetNuiFocus(false, false);
            API.SetNuiFocusKeepInput(false);
            NUIManager.Instance.CloseNUI();
        }

        private void SendNUIMessage(object data)
        {
            string json = Newtonsoft.Json.JsonConvert.SerializeObject(data);
            Debug.WriteLine($"[CORE] Envoi NUI message: {json.Substring(0, Math.Min(200, json.Length))}...");
        ...
        public async Task OnPlayerDeath()
        {
            ...
            {
                if (GetEntityHealth(GetPlayerPed(-1)) > 100 && !IsDead)
                {
                    StopScreenEffect("DeathFailMPIn");
                    ClearPedTasksImmediately(GetPlayerPed(-1));
